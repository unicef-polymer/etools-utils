...
import EtoolsAjaxDataMixin from './etools-ajax-data-mixin';
import {
  requestIsCacheable,
  getFromCache,
  cacheEndpointResponse
} from '../dexie-caching.util';
...
export class EtoolsRequestError extends Error {
  error: any;
  status: number;
  statusText: string;
  response: any;
 ...
 */
export function EtoolsAjaxRequestMixin<T extends Constructor  <any>>(baseClass: T){
        class EtoolsAjaxRequestMixinClass extends EtoolsAjaxDataMixin(baseClass) {

          @property({ type: Object}) lastAjaxRequest?: EtoolsXhrRequest;
          ...
          @property({ type: Array }) activeAjaxRequests: Array<any> = [];
            ...
            _setLastAjaxRequest (request: EtoolsXhrRequest) {
            ...
            _doRequest(reqConfigOptions: any, activeReqKey: any) {
                const request = /** @type {!EtoolsXhrRequest} */ (document.createElement('etools-request')) as EtoolsXhrRequest;
